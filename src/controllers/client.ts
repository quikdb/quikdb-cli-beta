...
export class QuikDB {
  ...
  private async loadConfiguration(): Promise<void> {
    try {
      ...
      const tokensData = fs.readFileSync((Tools.TOKEN_FILE), 'utf-8');
      ...
      const urlData = fs.readFileSync(Tools.URL_FILE, 'utf-8');
      console.log('Current Configuration:', urlData);

      const urlJson = Tools.getConfigAsJson(urlData) as AccessTokenType;

      const { url } = urlJson;

      const canisterIdData = fs.readFileSync(Tools.CANISTER_FILE, 'utf-8');
      console.log('Current Configuration:', canisterIdData);

      const canisterIdJson = Tools.getConfigAsJson(canisterIdData) as AccessTokenType;

      const { canisterId } = canisterIdJson;
